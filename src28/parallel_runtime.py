...
class ParallelGSPOPolicyProxy:
    def __init__(self, policy_key, client):
        self.policy_key = policy_key
        self.client = client
...
def _apply_worker_module_overrides(overrides, policy=None):
    ...
    for name, value in dict(overrides).items():
        ...
        for module_name in ("src28.config", "src28.model_loader", "src28.gspo_verl", "run_all_28"):
            module = sys.modules.get(module_name)
            if module is not None:
                setattr(module, name, copied_value)

    if policy is not None:
        gspo_module = sys.modules.get("src28.gspo_verl")
        ...
        response_queue.put(
            {
                ...
                "max_new_tokens": getattr(sys.modules.get("src28.gspo_verl"), "MAX_NEW_TOKENS", None),
                "middle_action_schema": getattr(
                    sys.modules.get("src28.gspo_verl"),
