...
from src28.data_loader import load_gsm8k_motac, load_gsm8k_splits, compute_accuracy, extract_pred_num
from src28.model_loader import generate_response, get_gpt2
from src28.gspo_verl import GSPOAgentPolicy
from src28.cfr_core import CFRBehaviorSelector
from src28.parallel_runtime import create_three_layer_parallel_controller
from src28.metrics_logger import (
    init_logs,
    log_single,
    log_polling,
    log_single_gspo,
...
from src28.plotter import plot_accuracy_comparison, plot_three_layer_details
...
def resolve_proposal_action_outcome(
    ...
    refresh_blocked_by_vote = (
        proposal_mode == "refresh_pending"
        and has_pending_candidate(pre_pending_candidate_text, pre_pending_candidate_pred)
        and int(pre_pending_vote_score or 0) > -1
    )
    if refresh_blocked_by_vote:
        return {
            "history": history,
            "incumbent_text": pre_incumbent_text,
            "incumbent_pred": pre_incumbent_pred,
...
def estimate_middle_action_values(
    ...
    if is_proposal_review_schema():
        ...
        context_override = build_sampling_context(
            ...
            pending_candidate_text=pending_candidate_text,
            pending_vote_score=pending_vote_score,
            include_latest_review_reason=(stage == "proposal"),
        )
        if MIDDLE_ACTION_COMMENT in allowed_actions:
...
def run_three_layer_proposal_review_action(
    ...
    context_override = build_sampling_context(
        ...
        include_latest_review_reason=(get_proposal_review_stage(rnd) == "proposal"),
    )
    candidate_outcomes = []
