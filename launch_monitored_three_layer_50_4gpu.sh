...
RUN_DIR="${MAS_RUN_DIR:-${PROJECT_ROOT}/runs/monitored_three_layer_50_${RUN_STAMP}}"
...
if [[ ! -x "${ENV_PYTHON}" ]]; then
  ...
  echo "请先创建环境，或通过 MAS_CONDA_ENV_PATH 指向正确的 conda 前缀。" >&2
  exit 1
fi

if [[ ! -f "${ITT_PRELOAD_SO}" ]]; then
  echo "找不到 ITT 共享库: ${ITT_PRELOAD_SO}" >&2
  echo "当前环境需要通过 LD_PRELOAD 预加载它，才能正常导入 torch。" >&2
  exit 1
fi
...
MAS_DEBUG_CUDA="${MAS_DEBUG_CUDA:-0}"
...
CMD+=("${ENV_PYTHON}" "${PROJECT_ROOT}/run_monitored_three_layer_50.py")
