...
def get_middle_allowed_actions(rnd, incumbent_pred, allow_silent=True, total_rounds=NUM_ROUNDS):
    ...
    if rnd >= total_rounds:
        ...
        if allow_silent:
            return [MIDDLE_ACTION_SILENT, MIDDLE_ACTION_ANSWER]
        return [MIDDLE_ACTION_ANSWER]
...
def prepare_three_layer_regret_context(
    ...
    if parallel_controller is not None and outer_cfr is not None and len(allowed_agents) > 1:
        ...
        with ThreadPoolExecutor(max_workers=len(allowed_agents)) as executor:
            for payload in executor.map(compute_agent_context, allowed_agents):
...
def record_three_layer_round(
    ...
    buffers["round_texts"][rnd-1].append(incumbent_text)
...
def run_policy_stack_experiment(
    ...
    resolved_parallel_mode = runtime.get("parallel_mode", parallel_mode)
            ...
            for branch_idx, branch_state in enumerate(active_branches):
                ...
                for child in branch_children:
                    round_records.append({
                        "incumbent_text": child["state"]["incumbent_text"],
