...
GSPO_FINETUNE_MODE = os.environ.get("MAS_GSPO_FINETUNE_MODE", "last_n_layers").strip().lower()
GSPO_TRAIN_LAST_N_LAYERS = 2       # 除lm_head外，再解冻最后N层decoder block
GSPO_LORA_R = int(os.environ.get("MAS_GSPO_LORA_R", "16") or "16")
GSPO_LORA_ALPHA = int(os.environ.get("MAS_GSPO_LORA_ALPHA", "32") or "32")
GSPO_LORA_DROPOUT = float(os.environ.get("MAS_GSPO_LORA_DROPOUT", "0.05") or "0.05")
...
# GSPO 在线更新的训练精度。
# 这台 4x H20 机器上，Phi-3 独立副本在 eager generate 路径里使用 bf16/fp16
# 会在首个 forward 直接触发原生 SIGFPE；float32 已验证可稳定跑通 sample_text。
# 因此这里默认切到 float32，优先保证训练链路先稳定跑起来。
#
# 可选值：
