...
class CFRBehaviorSelector:
    ...
    def update_regret(
        ...
        baseline_strategy = self.get_current_strategy(
            state_key,
            allowed_actions,
        )
        baseline_value = float(np.dot(action_values[mask], baseline_strategy[mask]))
        regrets = np.zeros(self.num_actions, dtype=np.float64)
        regrets[mask] = action_values[mask] - baseline_value
        ...
        if strategy_sum_override is None:
            ...
            strategy = np.asarray(strategy_sum_override, dtype=np.float64)
            if strategy.shape != (self.num_actions,):
                raise ValueError("strategy_sum_override 维度不正确。")
            strategy = np.maximum(strategy, 0.0) * mask
            total = strategy.sum()
            if total > 0:
                strategy = strategy / total
            else:
                strategy = self.get_current_strategy(state_key, allowed_actions)
            self.strategy_sum_by_state[key] += strategy
